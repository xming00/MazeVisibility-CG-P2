...
void Maze::draw_cell(Cell* now_cell, LineSeg L, LineSeg R) {
	...
	for (int i = 0; i < 4; i++) {
		...
		*/
		...
		else   // 透明牆的部分: run cell
		{
			if (now_cell->edges[i]->Neighbor(now_cell) == NULL) continue;			//切完後的牆不用處理
			// 左near點與右near點(廣義來說)
			...
			// base part
			// 下一個cell沒有被run到
			if (!now_cell->edges[i]->Neighbor(now_cell)->foot_print) {
